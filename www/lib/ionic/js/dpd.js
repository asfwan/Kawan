...
dpd.money = dpd("/money");
dpd.money.on = function(ev, fn) {
  return dpd.on("money" + ":" + ev, fn);
}
dpd.money.once = function(ev, fn) {
  return dpd.once("money" + ":" + ev, fn);
}
dpd.money.off = function(ev, fn) {
  return dpd.off("money" + ":" + ev, fn);
}
dpd.money.rename = function (path, body, fn) {
  dpd.money.exec("rename", path, body, fn);
}

dpd.sani = dpd("/sani");
dpd.sani.on = function(ev, fn) {
  return dpd.on("sani" + ":" + ev, fn);
}
dpd.sani.once = function(ev, fn) {
  return dpd.once("sani" + ":" + ev, fn);
}
dpd.sani.off = function(ev, fn) {
  return dpd.off("sani" + ":" + ev, fn);
}
dpd.sani.rename = function (path, body, fn) {
  dpd.sani.exec("rename", path, body, fn);
}

dpd.test = dpd("/test");
dpd.test.on = function(ev, fn) {
  return dpd.on("test" + ":" + ev, fn);
}
dpd.test.once = function(ev, fn) {
  return dpd.once("test" + ":" + ev, fn);
}
dpd.test.off = function(ev, fn) {
  return dpd.off("test" + ":" + ev, fn);
}
dpd.test.rename = function (path, body, fn) {
  dpd.test.exec("rename", path, body, fn);
}
...
dpd.userscrowdify = dpd("/users-crowdify");
dpd.userscrowdify.login = function(path, body, fn) {
  return dpd.userscrowdify.exec("login", path, body, fn);
}
dpd.userscrowdify.logout = function(path, body, fn) {
  return dpd.userscrowdify.exec("logout", path, body, fn);
}
dpd.userscrowdify.me = function(path, query, fn) {
  return dpd.userscrowdify.get("me", path, query, fn);
}
dpd.userscrowdify.on = function(ev, fn) {
  return dpd.on("users-crowdify" + ":" + ev, fn);
}
dpd.userscrowdify.once = function(ev, fn) {
  return dpd.once("users-crowdify" + ":" + ev, fn);
}
dpd.userscrowdify.off = function(ev, fn) {
  return dpd.off("users-crowdify" + ":" + ev, fn);
}

dpd.wanda = dpd("/wanda");
dpd.wanda.on = function(ev, fn) {
  return dpd.on("wanda" + ":" + ev, fn);
}
dpd.wanda.once = function(ev, fn) {
  return dpd.once("wanda" + ":" + ev, fn);
}
dpd.wanda.off = function(ev, fn) {
  return dpd.off("wanda" + ":" + ev, fn);
}
dpd.wanda.rename = function (path, body, fn) {
  dpd.wanda.exec("rename", path, body, fn);
}
